...
export function Videography() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Promote GPU layer before animation
      gsap.set(reveals, {
        opacity: 0,
        y: 40,
        filter: "blur(12px)",
      ...
      gsap.to(reveals, {
        ...
        ease: "expo.out",
        stagger: 0.08,
        // Remove GPU layer when animation finishes
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Promote wrapper layer natively in fromVars
      const fromVars = {
        opacity: 0,
        y: 80,
        scale: 0.95,
      ...
      if (eagerRows.length) {
        gsap.fromTo(eagerRows, fromVars, {
          ...toVars,
          // Release GPU layer right when the row's animation finishes
    ...
    >
      <div ref={root} className="max-w-[1600px] mx-auto">
        ...
        {isMobile ? (
          ...
          >
            {PROJECTS.map((p) => (
              <VideoCard key={p.id} p={p} onOpen={setActive} compact />
            ))}
          </div>
        ) : (
...
function VideoCard({ p, onOpen, compact = false }) {
  return (
    <button
      type="button"
      onClick={() => onOpen(p)}
