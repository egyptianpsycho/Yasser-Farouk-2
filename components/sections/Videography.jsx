...
export function Videography() {
  ...
  // Row-grouped scroll-in animation
  useEffect(() => {
  ...
  return <section id="motion" className="py-24 md:py-32 px-4 sm:px-6 border-t border-border">
      <div ref={root} className="max-w-[1600px] mx-auto">
        <div className="flex justify-between items-end mb-12 md:mb-16 gap-4">
          <div className="min-w-0">
            <span data-reveal className="font-mono text-xs uppercase text-primary block">
              Videography
            </span>
            <h2 ref={titleRef} data-reveal className="font-display uppercase tracking-tighter leading-none mt-2" style={{
            fontSize: "clamp(3rem, 12vw, 8rem)"
          }}>
              Vision in <span className="italic text-primary">motion</span>
            </h2>
          </div>
          <div data-reveal className="font-mono text-[10px] sm:text-xs uppercase text-muted-foreground hidden sm:block shrink-0">
            [ {String(PROJECTS.length).padStart(2, "0")} ] Selected Works
          </div>
        </div>

        {isMobile ? <div ref={gridRef} style={{
        display: "grid",
        ...
        marginLeft: "calc(-50vw + 50%)"
            ...
            {PROJECTS.map(p => <VideoCard key={p.id} p={p} onOpen={setActive} compact />)}
          </div> : <div ref={gridRef} className="parent-video mx-auto">
            {PROJECTS.map((p, idx) => <div key={p.id} className={`video-item div${idx + 1}-video relative overflow-hidden`}>
                <VideoCard p={p} onOpen={setActive} />
              </div>)}
          </div>}
      </div>

      <VideoViewer open={!!active} project={active} onClose={() => setActive(null)} />
    </section>;
}
function VideoCard({
  p,
