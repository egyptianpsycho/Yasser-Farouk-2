    ...
    >
      <div className="relative w-14 h-14">
        <svg viewBox="0 0 56 56" className="absolute inset-0 w-full h-full">
          <circle cx="28" cy="28" r="24" fill="none" stroke="#C41E3A" strokeWidth="1" opacity="0.08" />
        </svg>
        <svg
          viewBox="0 0 56 56"
        ...
        >
          <circle cx="28" cy="28" r="24" fill="none" stroke="rgba(201,168,76,0.10)" strokeWidth="1.5" />
          <path d="M28 4a24 24 0 0 1 24 24" stroke="#C41E3A" strokeWidth="1.5" fill="none" strokeLinecap="round" />
        </svg>
      </div>
      {title && (
        <p
...
function NativePlayer({ src, onOrientation }) {
  ...
  // ── Bulletproof responsive player sizing for Safari ───────────────────────
  // Uses purely width driven calculations bounded by screen height equivalents
  // to avoid all `aspect-ratio` + `height: 100%` bugs on iOS browsers.
  const playerStyle =
    orientation === null
      ...
      ? {
          width: "100%",
          maxWidth: "min(calc(85vh * 0.5625), 520px)",
          aspectRatio: "9/16",
          margin: "0 auto",
        }
      : orientation === "square"
  ...
  return (
    <div
      ref={containerRef}
      className="relative bg-black select-none shadow-2xl rounded-sm overflow-hidden"
      style={playerStyle}
...
export function VideoViewer({ open, project, onClose }) {
  ...
  useEffect(() => {
    ...
    if (project && isEmbeddable(project.video)) {
      gsap.fromTo(
        ...
        { opacity: 1, filter: "blur(0px)", x: 0, duration: 0.6, ease: "power3.out", delay: 0.25 }
      );
    }
  }, [open]);
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      headerTitleRef.current,
      { opacity: 0, filter: "blur(12px)", x: -20 },
      { opacity: 1, filter: "blur(0px)", x: 0, duration: 0.6, ease: "power3.out", delay: 0.05 }
    );
  }, [viewerOrientation]);
  ...
  return createPortal(
    <div ref={overlayRef} className="fixed inset-0 z-[9998] bg-black flex flex-col" style={{ opacity: 0 }}>
      {/* ── Entry loading overlay ──────────────────────────────────────────── */}
      <EntryLoader loaderRef={entryLoaderRef} title={project.title} />

      {/* ── Header bar ────────────────────────────────────────────────────── */}
      <div className="flex items-center gap-3 px-5 sm:px-8 py-5 shrink-0 border-b border-white/5">
        <button
          onClick={handleClose}
        ...
        <div className="min-h-full w-full flex flex-col md:flex-row items-center md:items-stretch justify-center gap-10 md:gap-16 px-5 py-12 md:py-16 md:px-12 mx-auto max-w-[1800px]">
          ...
          >
            <div className="flex flex-col gap-4">
              ...
              <h2 className="font-display text-4xl lg:text-6xl uppercase tracking-tighter leading-[0.9] text-white break-words">
                {project.title}
              </h2>
              
              <div className="w-8 h-[2px] bg-primary hidden md:block mt-1 mb-1" />

              {/* Injected the description handling here */}
              {project.description && (
                <p className="font-sans text-sm md:text-base text-white/70 leading-relaxed max-w-md mt-2 ">
                  {project.description}
                </p>
              )}

              {project.meta && (
                <p className="font-mono text-[10px] md:text-xs uppercase tracking-widest text-white/40 mt-4 md:mt-2">
                  {project.meta}
                </p>
              )}
            </div>
...
//       : { width: "min(80vw,1200px)", aspectRatio: "16/9" };
