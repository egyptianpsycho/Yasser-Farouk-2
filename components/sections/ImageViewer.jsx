...
function ImageLightbox({
  ...
  return createPortal(<div ref={overlayRef} onClick={onClose} className="fixed inset-0 z-[10000] bg-black/95 backdrop-blur-md flex items-center justify-center p-4 md:p-10" style={{
    opacity: 0
  }}>
      <button onClick={onClose} className="absolute top-4 right-4 md:top-6 md:right-6 size-10 md:size-12 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-mono text-xs uppercase tracking-widest hover:scale-110 transition-transform z-20" aria-label="Close">
        ✕
      </button>
...
export function ImageViewer({
  ...
  const allImages = project ? project.cover ? [project.cover, ...project.images.filter(img => img !== project.cover)] : project.images : [];
  useEffect(() => {
    ...
    if (loaderRef.current) gsap.set(loaderRef.current, {
      opacity: 1,
      scale: 1
    });
    ...
    if (centerTitleRef.current) gsap.set(centerTitleRef.current, {
      opacity: 0,
      filter: "blur(40px)",
      scale: 1.1
    });
    if (headerTitleRef.current) gsap.set(headerTitleRef.current, {
      opacity: 0,
      filter: "blur(12px)"
    });
  ...
  useEffect(() => {
    if (!open) return;
    const onKey = e => {
      if (e.key === "Escape" && !imageLightbox) handleClose();
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [open, imageLightbox, handleClose]);
  if (!mounted || !open || !project) return null;
  const progress = totalRef.current > 0 ? Math.round(loadedCount / totalRef.current * 100) : 0;
  return createPortal(<div ref={overlayRef} className="fixed inset-0 z-[9998] bg-black/95 backdrop-blur-sm" style={{
    opacity: 0
  }}>
      {/* Loader */}
      <div ref={loaderRef} className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-8 pointer-events-none">
        <div className="relative w-24 h-24 flex items-center justify-center">
          ...
          <svg className="absolute inset-0 w-full h-full -rotate-90 animate-spin" style={{
      ...
      <div ref={centerTitleRef} className="absolute inset-0 z-30 flex items-center justify-center pointer-events-none px-6" style={{
      opacity: 0,
      filter: "blur(40px)"
    }}>
        <h1 className="font-display text-center leading-none uppercase tracking-tighter select-none" style={{
      ...
      {/* Content */}
      <div ref={contentRef} className="w-full h-full flex flex-col" style={{
      opacity: 0
    }}>
        {/* Header */}
        <div className="flex items-center gap-3 sm:gap-4 px-4 sm:px-8 pt-6 pb-4 border-b border-border bg-background/80 backdrop-blur-md shrink-0">
          ...
          <h2 ref={headerTitleRef} className="font-display text-base sm:text-xl uppercase tracking-wider truncate" style={{
          opacity: 0,
          filter: "blur(12px)"
        }}>
            {project.title}
        ...
        {/* Masonry grid */}
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 sm:p-4" style={{
        ...
        touchAction: "pan-y"
          ...
          columnGap: "8px"
            ...
            {allImages.map((src, idx) => <button key={idx} type="button" className="viewer-img-item group block w-full cursor-pointer relative overflow-hidden border border-border mb-2 text-left" style={{
            breakInside: "avoid"
          }} onClick={() => setImageLightbox({
            index: idx
          })}>
                <Image src={src} alt={`${project.title} ${idx + 1}`} width={0} height={0} sizes="(max-width: 768px) 100vw, 33vw" className="w-full h-auto block group-hover:scale-105 transition-transform duration-700 ease-out" onLoad={handleImageLoad} onError={handleImageLoad} />
      ...
      {imageLightbox && <ImageLightbox images={project.images} cover={project.cover} startIndex={imageLightbox.index} onClose={() => setImageLightbox(null)} />}
    </div>, document.body);
}
