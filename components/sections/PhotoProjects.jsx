...
const CATEGORIES = ["All", "Commercial", "F&B", "Jewelry"];
...
export function PhotoProjects() {
  ...
  const filtered =
    displayed === "All"
      ? PROJECTS
      : PROJECTS.filter((p) => p.category === displayed);
  ...
  return (
    <section id="projects" className="py-24 md:py-32 px-4 sm:px-6 ">
      <div className=" mx-auto">
        ...
        >
          <div>
            ...
            <h2
              data-reveal
              className="font-display  uppercase leading-[0.85] tracking-tighter mt-3"
              style={{
          ...
          <p
            data-reveal
            className="max-w-sm font-mono text-[10px] sm:text-xs uppercase tracking-widest text-muted-foreground leading-loose"
          >
            A living index of recent photographic series. Tap any tile to enter
            the full set — frames, locations and the in-between moments.
          </p>
        </div>
        ...
        <div data-reveal className="mb-6 relative z-40">
          <div ref={dropdownRef} className="relative inline-block">
            <button
              ...
              className="flex items-center gap-3 px-4 py-2 border border-border font-bold  bg-card/60  text-foreground/80 hover:text-foreground hover:border-primary duration-300 transition-colors font-mono text-[10px] sm:text-xs uppercase tracking-[0.25em]"
            >
              {selected}
              <svg
                className={`w-3 h-3 transition-transform duration-300 opacity-60 ${
            ...
            <div
              className={`absolute  top-full left-0 mt-2 w-48 border border-border bg-background/95 backdrop-blur-xl overflow-hidden shadow-2xl transition-all duration-300 origin-top-left ${
                dropdownOpen
            ...
            >
              {CATEGORIES.map((cat) => (
                <button
                  ...
                  className={`w-full px-4 py-3 text-left font-mono text-[10px] uppercase tracking-[0.25em] transition-all duration-200 ${
                    selected === cat
                      ? "text-foreground bg-primary/15 border-l-2 border-primary"
        ...
        {isMobile ? (
          <div
            ...
            className={`transition-grid ${
              transitioning ? "grid-exit" : "grid-enter"
            }`}
            style={{
              display: "grid",
          ...
          >
            {filtered.map((p) => (
              <div
                key={p.id}
                onClick={() => setActive(p)}
                className="photo-item relative aspect-square overflow-hidden bg-secondary cursor-pointer group"
              >
                <OptimizedImage
                  src={p.cover}
                  alt={p.title}
          ...
          <div
            ref={gridRef}
            className={`parent transition-grid  ${
              transitioning ? "grid-exit" : "grid-enter"
            } ${displayed !== "All" ? "filtered" : ""}`}
          >
            {filtered.map((p, idx) => (
              <div
                key={p.id}
                ...
                className={`photo-item div${
                  idx + 1
                } relative cursor-pointer group overflow-hidden bg-secondary`}
              >
                <OptimizedImage
                  src={p.cover}
                  alt={p.title}
                  priority={true}
                  sizes="(max-width: 768px) 50vw, 25vw"
                  className="object-cover bg-red-500 transition-transform duration-1000 ease-out group-hover:scale-110"
                />
      ...
      <ImageViewer
        open={!!active}
        project={active ? toViewerProject(active) : null}
